...
function convert() {
    ...
    var xmlNew = '<!DOCTYPE codename-engine-character> <!-- Made with WizardMantis\'s Character Converter that was modified by inky lol -->\n<character isPlayer="false" x="'+json.position[0]+'" y="'+json.position[1]+'" icon="'+json.healthicon+'" flipX="'+json.flip_x+'" camx="'+json.camera_position[0]+'" camy="'+json.camera_position[1]+'" holdTime="'+json.sing_duration+'" scale="'+json.scale+'"> color="'+FlxColor.fromRGB(json.healthbar_colors[0], json.healthbar_colors[1], json.healthbar_colors[2])+'"\n';
